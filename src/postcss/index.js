...
void (async () => {
    ...
    const { data: cssFiles, error } = await attemptAsync(async () => await glob('src/composite/*.css', { 
        ignore: 'node_modules'
    }));
    ...
    for (const file of cssFiles) {
        ...
        const { data: css, error: cssFileReadError } = attempt(fs.readFileSync.bind(this,  filePath, { encoding: 'utf-8' }))
        if (cssFileReadError || !css) {
            logger.fatal(cssFileReadError);
            process.exit(1);
        }
        await attemptAsync(
          async () => await postcss([cssnano])
